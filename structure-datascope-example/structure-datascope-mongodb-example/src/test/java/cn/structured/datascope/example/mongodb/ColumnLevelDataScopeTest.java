package cn.structured.datascope.example.mongodb;
 ...
 * - 通过 MockMvc 调用 /api/orders/list 接口（直接返回 List，不经过 ResResultVO 包装）
...
class ColumnLevelDataScopeTest {
    ...
    class ApiResponseStructureTest {
        ...
        void verifyApiResponseStructure() throws Exception {
            ...
            // 验证响应包含订单数据
            assertTrue(content.contains("orderNo"), "Response should contain orderNo field");
        }
        ...
        void verifyResponseNotBlocked() throws Exception {
            ...
            // 验证响应包含数据
            assertTrue(content.contains("orderNo"), "Response should still return data even with hidden fields");
        }
    }
}
