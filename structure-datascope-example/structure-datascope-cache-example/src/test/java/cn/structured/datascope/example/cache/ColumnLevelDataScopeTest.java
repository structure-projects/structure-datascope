...
import cn.structured.datascope.example.cache.config.MockRedisConfiguration;
    ...
    class ApiResponseStructureTest {
        ...
        void verifyApiResponseStructure() throws Exception {
            ...
            mockMvc.perform(get("/api/orders")
                    ...
                    .andDo(result -> System.out.println("Response structure: " + result.getResponse().getContentAsString()));
        }
        ...
        void verifyResponseNotBlocked() throws Exception {
            ...
            mockMvc.perform(get("/api/orders")
                    ...
                    .andExpect(jsonPath("$.success").value(true))
                    .andExpect(jsonPath("$.data").isArray())
                    .andDo(result -> System.out.println("Non-blocked response: " + result.getResponse().getContentAsString()));
        }
    }

    @Nested
    @DisplayName("缓存数据权限隔离验证")
    class CacheDataScopeIsolationTest {

        @Test
        @DisplayName("不同组织用户 - 缓存数据隔离")
        void differentOrg_CacheIsolation() throws Exception {
            // 组织10用户获取数据
            setupUserContext("user-row-org10-admin");

            MvcResult resultOrg10 = mockMvc.perform(get("/api/orders")
                            .header("X-User-Id", "user-row-org10-admin"))
                    .andExpect(status().isOk())
                    .andReturn();

            System.out.println("Org10 data: " + resultOrg10.getResponse().getContentAsString());

            // 切换到组织20
            DataScopeContext.remove();
            setupUserContext("user-row-org20-admin");

            MvcResult resultOrg20 = mockMvc.perform(get("/api/orders")
                            .header("X-User-Id", "user-row-org20-admin"))
                    .andExpect(status().isOk())
                    .andReturn();

            System.out.println("Org20 data: " + resultOrg20.getResponse().getContentAsString());

            // 验证两个组织的数据隔离（缓存键前缀不同）
            // 由于缓存键前缀包含 orgId，不同组织的数据不会互相干扰
        }

        @Test
        @DisplayName("不同部门用户 - 缓存数据隔离")
        void differentDept_CacheIsolation() throws Exception {
            // 部门1用户获取数据
            setupUserContext("user-row-dept1");

            MvcResult resultDept1 = mockMvc.perform(get("/api/orders")
                            .header("X-User-Id", "user-row-dept1"))
                    .andExpect(status().isOk())
                    .andReturn();

            System.out.println("Dept1 data: " + resultDept1.getResponse().getContentAsString());

            // 切换到部门1和2主管
            DataScopeContext.remove();
            setupUserContext("user-row-dept1-2-manager");

            MvcResult resultDept12 = mockMvc.perform(get("/api/orders")
                            .header("X-User-Id", "user-row-dept1-2-manager"))
                    .andExpect(status().isOk())
                    .andReturn();

            System.out.println("Dept1-2 manager data: " + resultDept12.getResponse().getContentAsString());
