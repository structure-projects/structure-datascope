...
class ColumnLevelDataScopeTest {
    ...
    @Nested
    @DisplayName("ResResultVO格式验证")
    class ResResultVOFormatTest {

        @Test
        @DisplayName("验证 API 响应使用 ResResultVO 格式")
        void verifyResResultVOFormat() throws Exception {
            setupUserContext("user-003");  // SYS_ADMIN 角色
            ...
            String content = result.getResponse().getContentAsString();
            System.out.println("ResResultVO format response: " + content);

            // 验证 ResResultVO 格式
            assertTrue(content.contains("\"success\":true"), "Response should use ResResultVO with success:true");
            assertTrue(content.contains("\"data\""), "Response should use ResResultVO with data field");
            assertTrue(content.contains("\"code\":0") || content.contains("\"code\":\"SUCCESS\""),
                    "Response should have proper code field");
        }

        @Test
        @DisplayName("验证直接返回List接口的列级权限过滤")
        void verifyDirectListColumnFiltering() throws Exception {
            setupUserContext("user-001");  // EMPLOYEE 角色

            // 通过 /api/orders/list 直接返回 List，测试列级过滤
            ...
            String content = result.getResponse().getContentAsString();
            System.out.println("Direct list response: " + content);

            // 直接返回 List 时，也应该进行列级过滤
            assertTrue(content.contains("\"email\""), "Direct list should still have email visible");
            assertTrue(content.contains("\"amount\":null") || content.contains("\"amount\":null"),
                    "Direct list should filter amount field");
        }
    }
}
